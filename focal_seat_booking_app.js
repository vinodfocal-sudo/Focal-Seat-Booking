...
/* ============================
   SUPABASE — centralised live database
   Loaded via CDN <script> in index.html (window.supabase).
   Strategy: localStorage keeps UI working instantly on load.
   Supabase is the single source of truth — on init we fetch
   from Supabase, overwrite localStorage, and re-render.
   Every write goes to BOTH localStorage (instant UI) AND Supabase.
============================= */
...
function initSupabase() {
  try {
    if (window.supabase && typeof window.supabase.createClient === 'function') {
      ...
      console.log('[FocalBooking] Supabase client ready.');
      return true;
    }
    console.warn('[FocalBooking] Supabase library not found — localStorage-only mode.');
    return false;
  } catch(e) {
    console.warn('[FocalBooking] initSupabase error:', e);
    return false;
  }
}
...
function syncFromSupabase() {
  if (!_sb) return;
  _sb.from('bookings').select('seat,date,initials').then(function(res) {
    if (res.error) { console.warn('[FocalBooking] Supabase fetch error:', res.error.message); return; }
    setData(KEYS.bookings, res.data || []);
    renderTable();
    console.log('[FocalBooking] Synced', (res.data||[]).length, 'bookings from Supabase.');
  });
}

function sbInsertBooking(seat, date, initials) {
  if (!_sb) return;
  _sb.from('bookings').insert([{seat:seat, date:date, initials:initials}]).then(function(res) {
    if (res.error) console.warn('[FocalBooking] Supabase insert error:', res.error.message);
  });
}

function sbUpdateBooking(seat, date, initials) {
  if (!_sb) return;
  _sb.from('bookings').update({initials:initials}).eq('seat',seat).eq('date',date).then(function(res) {
    if (res.error) console.warn('[FocalBooking] Supabase update error:', res.error.message);
  });
}

function sbDeleteBooking(seat, date) {
  if (!_sb) return;
...
function sbDeleteAllBookings() {
  if (!_sb) return;
  _sb.from('bookings').delete().neq('seat','').then(function(res) {
    if (res.error) console.warn('[FocalBooking] Supabase deleteAll error:', res.error.message);
  });
}

function sbSubscribeBookings() {
  if (!_sb) return;
  try {
    _sb.channel('bookings-realtime')
      .on('postgres_changes', {event:'*', schema:'public', table:'bookings'}, function() {
        console.log('[FocalBooking] Real-time update — re-syncing.');
        syncFromSupabase();
      })
      .subscribe(function(status) {
        console.log('[FocalBooking] Realtime status:', status);
      });
  } catch(e) {
    console.warn('[FocalBooking] sbSubscribeBookings error:', e);
  }
}
...
function confirmBooking() {
  ...
  sbInsertBooking(pendingCell.seat, pendingCell.date, initials);
}
